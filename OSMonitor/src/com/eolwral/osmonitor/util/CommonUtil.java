...
public class CommonUtil {
  ...
  public static boolean execCore(Context context) {
    ...
    // execute osmcore
    try {
      Runtime.getRuntime().exec(new String[] { "chmod", "755", binary })
          .waitFor();
      ...
      if (!settings.isRoot()) {
        Runtime
            .getRuntime()
            .exec(
                new String[] { "sh", "-c", binary + " " + binary + ".token &" })
            .waitFor();
      } else {
        if (!isLollipop()) {
          Runtime
              .getRuntime()
              .exec(
                  new String[] { "su", "-c", binary + " " + binary + ".token &" })
              .waitFor();
        } else {
          ...
          os.writeBytes("su --context u:r:init:s0 -c \"" + binary + "_L " + binary + ".token &\" \n");
          os.writeBytes("exit\n\n");
          os.flush();
          Thread.sleep(1000);
          process.destroy();
